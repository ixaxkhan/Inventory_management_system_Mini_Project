   ...
   DateTimeFormatter dateformat = DateTimeFormatter.ofPattern("yyyy-MM-dd");
    ...
    private void UpdatePurchcase(ActionEvent event) {
      ...
      pName.setText(itemData.get(1));
        ...
        String[] pdate = itemData.get(5).split(" ");  // get only date from DateTime
               pDate.setValue(LocalDate.parse(pdate[0],dateformat));
        String[] edate = itemData.get(8).split(" ");  // get only date from DateTime
               eDate.setValue(LocalDate.parse(edate[0],dateformat));
